...
class PTZCamera():
    def __init__(self, map_size, grid_size, general_properties,
        ...
        if self.valid_sensors['camera']:
            ...
            self.angle_of_view      = camera_properties['angle of view']/180*np.pi
    ...
    def RosInit(self):
        ...
        #####  出錯  #### trospy.Subscriber("ground_truth/state", Odometry, self.PosCallback)   ##############  
        rospy.Subscriber(f"/agent_{self.id}/odom", Odometry, self.PosCallback)
   ...
   ############################有改######################
    def UpdatePosition(self, u_p):
        # 防止除以零錯誤
        norm_u_p = np.linalg.norm(u_p)
        if norm_u_p == 0:
            norm_u_p = 1e-6  # 加入極小值以避免除以零

        # 正規化速度
        u_p = self.max_speed * (u_p / norm_u_p)
        ...
        # 檢查 NaN 與更新位置
        if not np.isnan(u_p).any() and not np.isinf(u_p).any():
            next_pos += self.K_p * u_p * self.step
        else:
            rospy.logwarn("Invalid control signal detected. Skipping update.")
            return  # 中止此次位置更新

        # 加入地圖邊界限制 (假設地圖尺寸 24x24)
        ...
        yaw_rate_denom = np.inner(np.array([math.cos(self.yaw), math.sin(self.yaw)]), u_p)
        yaw_rate = 0

        # 防止除以零的情況
        if yaw_rate_denom != 0:
            yaw_rate = np.arctan(
                np.inner(np.array([-math.sin(self.yaw), math.cos(self.yaw)]), u_p) / yaw_rate_denom
            ) * self.step

        # 碰撞避免條件
        num_neighbors = len(self.neighbors_buffer)
        num_variables = 2
        ...
        if num_neighbors > 0:
            ...
            for i, neighbor in enumerate(self.neighbors_buffer):
                ...
                g[i, 0] = 2 * (x_diff + 0.1 * math.cos(self.yaw)) * math.cos(self.yaw) + \
                        2 * (y_diff + 0.1 * math.sin(self.yaw)) * math.sin(self.yaw)
                g[i, 1] = -2 * (x_diff + 0.1 * math.cos(self.yaw)) * 0.1 * math.sin(self.yaw) + \
                        2 * (y_diff + 0.1 * math.sin(self.yaw)) * 0.1 * math.cos(self.yaw)

                h[i] = 3 * (x_diff ** 2 + y_diff ** 2 - 0.3 ** 2)  # 安全距離限制
            ...
            try:
                sol = solvers.coneqp(P, Q, G, H, verbose=False)
                u_opt = sol['x']

                # 檢查解的合法性
                if np.isnan(u_opt).any() or np.isinf(u_opt).any():
                    rospy.logwarn("Invalid solution detected. Resetting control.")
                ...
                rospy.logerr(f"QP Solver Failed: {e}")
                cmd = Twist()
                cmd.linear.x = 0
                cmd.angular.z = 0
                self.pub_cmd_vel.publish(cmd)
    ...
    def UpdateSensorVoronoi(self, role, event):
        ...
        if role in target[5]:
            ...
            for neighbor in self.sensor_graph[role]:
                ...
                cost = (((pos_self[0]/grid_size[0] - x_coords)**2 + (pos_self[1]/grid_size[1] - y_coords)**2) - (100*self.sensor_weight[role][event])**2)*global_event#self.ComputeCost(role, pos_self, grid_size, x_coords, y_coords)# - (100*self.neighbors[neighbor]["weights"][role][event])**2
            ...
            for event in self.targets.keys():
                for sensor in self.valid_sensors.keys():
                    ...
                    if self.valid_sensors[sensor] and sensor in self.targets[event][5]:
                        ...
                        self.sensor_weight[sensor][event] = (0.0001 + self.sensor_scores[sensor][event])/(0.0001 + self.sensor_scores[sensor][event] + single_sensor_sum)
    ...
    def ComputeUtility(self):
        ...
        for event in self.targets.keys():
            for role in self.valid_sensors.keys():
                if role == "smoke_detector":
                    continue  # 跳過 smoke_detector，不計算
                ...
                if self.valid_sensors[role] and role in self.targets[event][5]:
                    ...
                    for k in self.valid_sensors.keys():
                        if k in self.targets[event][5] and k != role:
                            ...
                            tmp *= (0 + self.w_coop*coop_quality)

                    tmp *= self.event_density[event]*self.grid_size[0]

                    self.total_score += np.sum(tmp) #self.sensor_weight[role][event]*np.sum(tmp)
                    ...
                    if self.cooperation == True:
                        for k in self.valid_sensors.keys():
                            if k in self.targets[event][5] and k != role:
                                ...
                                sensor_gradient[0] *= (0 + self.w_coop*coop_quality)
                                sensor_gradient[1] *= (0 + self.w_coop*coop_quality)

                                if role == 'camera':
                                    perspective_gradient_x *= (0 + self.w_coop*coop_quality)
                                    perspective_gradient_y *= (0 + self.w_coop*coop_quality)
    ...
    def ComputeGradient(self, role, event, type):
        ...
        if role == "camera":
            ...
            if type == 'x':
                ...
                per_quality = (1/(1-np.cos(self.angle_of_view)))*((x*self.perspective[0] + y*self.perspective[1])/(dist+0.001) - np.cos(self.angle_of_view))
        ...
        if role == 'camera':
            ...
            for neighbor in self.sensor_graph[role]+[self.id]:
                ...
                if valid:
                    ...
                    x = (x_coords*grid_size[0] - pos_self[0])
                    y = (y_coords*grid_size[1] - pos_self[1])
                    per_quality = (1/(1-np.cos(self.angle_of_view)))*((x*self.perspective[0] + y*self.perspective[1])/(0.001+dist) - np.cos(self.angle_of_view))
...
if __name__ == "__main__":
    ...
    np.random.seed(id + 5*trial_id)
    pos_x                   = rospy.get_param("~pos_x", default=0)
    pos_y                   = rospy.get_param("~pos_y", default=0)
    init_position           = np.array([pos_x, pos_y])#(np.random.random((1,2))*2)[0] + 0  #
    ...
    UAV_self = PTZCamera(map_size = map_size, grid_size = grid_size, general_properties=general_info,
                        ...
                        manipulator_properties=manipulator_info, coop = coop, balance = balance, strength = w_coop)
    
    rospy.Subscriber("/kill", Int16, KillCB)
    rospy.Subscriber("/agent_"+str(id)+"/failure", Int16, FailureCB)
    rospy.Subscriber("/record_clk", Int16, record_clk_cb)
    ...
    while not rospy.is_shutdown() and not kill and not failure:
        if tick and UAV_self.start:
            ...
            per_x.append(UAV_self.perspective[0])
    ...
    if coop and balance:
        suffix = "/coop/balance/"
    elif coop and not balance:
        suffix = "/coop/unbalance/"
    elif not coop and balance:
        suffix = "/non-coop/balance/"
    else:
        suffix = "/non-coop/unbalance/"
        
    print(f"Frame: {cnt}, Score: {UAV_self.total_score}, Pos: {UAV_self.pos}")
    
    save_path = "/home/heish/research_ws/src/voronoi_cbsa/result/" + str(trial_id) + suffix
